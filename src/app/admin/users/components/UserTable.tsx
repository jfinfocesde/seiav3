...
export function UserTable({ users = [], areas }: UserTableProps) {
  ...
  const openEditModal = (user: DisplayUser) => {
    setEditUser(user);
    setEditAreaId(user.areaId ? String(user.areaId) : '');
  ...
  const handleSave = async () => {
    ...
    const data = await updateUserFull({
      ...
      areaId: editUser.role === 'TEACHER' && editAreaId ? Number(editAreaId) : null,
    ...
    <div className="border rounded-lg w-full overflow-x-auto">
      <div className="flex items-center gap-2 p-4">
        ...
        >
          ...
          {areas.map(area => (
            <option key={String(area.id)} value={String(area.id)}>{area.name}</option>
          ))}
        </select>
      </div>
