...
interface SubmissionSummary {
  ...
  timeOutsideEval: number; // in seconds
}
...
export async function generateScheduleAnalysis(
  ...
  try {
    ...
    const submissionsDetail = submissions.map(s => 
      `- ${s.studentName}: Calificación ${s.score?.toFixed(1) ?? 'N/A'}, Intentos de Fraude: ${s.fraudAttempts}, Tiempo Fuera: ${(s.timeOutsideEval / 60).toFixed(1)} min`
    ).join('\n');
    ...
    - Total de Intentos de Fraude: ${stats.totalFraudAttempts}
 ...
 * @param participationData - Array de objetos { studentName, score, timeOutsideEval }
...
export async function generatePersonalizedRecommendations(
  ...
  for (const student of studentsData) {
    ...
    INTENTOS DE FRAUDE: ${student.fraudAttempts}
    TIEMPO FUERA: ${(student.timeOutsideEval / 60).toFixed(1)} min
