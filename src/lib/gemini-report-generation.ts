...
/**
 * @returns Un reporte detallado en formato Markdown.
...
export async function generateReport(
  ...
  try {
    ...
    const answersDetail = answers.map((answer, index) => `
      Pregunta ${index + 1} (${answer.questionType}):
      ${answer.questionText}
      Respuesta del estudiante: ${answer.studentAnswer}
      Calificación: ${answer.score !== null ? answer.score : 'No evaluada'}/5.0
    `).join('\n');
    ...
    const jsonMatch = text.match(/\{[\s\S]*\}/);
    if (jsonMatch) {
        return JSON.parse(jsonMatch[0]) as ReportResult;
    }
    
    // Fallback si no hay JSON
    return {
        overallFeedback: `Reporte no pudo ser generado por la IA. Calificación final: ${averageScore.toFixed(1)}/5.0.`,
        strengths: [],
        areasForImprovement: [],
        grade: averageScore,
    };
    ...
    return {
      overallFeedback: 'Ocurrió un error al generar el reporte.',
      strengths: [],
      areasForImprovement: [],
      grade: averageScore,
    };
