...
export function FraudReflectionModal({
  ...
  useEffect(() => {
    if (isOpen) {
      setLoading(true);
      generateFraudReflection(fraudType, fraudCount, 0, evaluationId)
        .then(result => {
        ...
        .catch(error => {
          ...
          setReflection({
            title: 'Error',
            message: 'No se pudo cargar la reflexión.',
            suggestion: 'Por favor, continúa.'
          });
        })
        .finally(() => {
          setLoading(false);
  ...
  }, [isOpen, fraudType, fraudCount, evaluationId]);
